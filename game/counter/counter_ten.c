...
/*   Updated: 2024/02/27 16:49:02 by amirloup         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ten_to_window(t_game *g, int x)
{
	if (mlx_image_to_window(g->mlx, g->co.t2, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t2->instances->enabled = false;
	if (mlx_image_to_window(g->mlx, g->co.t3, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t3->instances->enabled = false;
	if (mlx_image_to_window(g->mlx, g->co.t4, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t4->instances->enabled = false;
	if (mlx_image_to_window(g->mlx, g->co.t5, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t5->instances->enabled = false;
	if (mlx_image_to_window(g->mlx, g->co.t6, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t6->instances->enabled = false;
	if (mlx_image_to_window(g->mlx, g->co.t7, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t7->instances->enabled = false;
	if (mlx_image_to_window(g->mlx, g->co.t8, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t8->instances->enabled = false;
	if (mlx_image_to_window(g->mlx, g->co.t9, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
...
void	get_ten(t_game *g)
{
	...
	resize_ten(g);
	if (mlx_image_to_window(g->mlx, g->co.t0, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
	g->co.t0->instances->enabled = true;
	if (mlx_image_to_window(g->mlx, g->co.t1, x, 20) == -1)
		exit((ft_printf("Error\nImage on window!\n"), d_a(g), EXIT_FAILURE));
