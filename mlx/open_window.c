...
/*   Updated: 2024/02/16 13:13:18 by amirloup         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move(mlx_key_data_t key, void *param)
{
	...
	if (key.key == MLX_KEY_W && key.action == 1 && check_up(g) == 1 && g->success == 0)
	{
		g->sprite.player->instances[0].y -= g->p_size_y;
		g->sprite.fo->instances[0].y -= g->p_size_y;
		g->count++;
	}
	if (key.key == MLX_KEY_S && key.action == 1 && check_down(g) == 1 && g->success == 0)
	{
		g->sprite.player->instances[0].y += g->p_size_y;
		g->sprite.fo->instances[0].y += g->p_size_y;
		g->count++;
	}
	if (key.key == MLX_KEY_A && key.action == 1 && check_left(g) == 1 && g->success == 0)
	{
		g->sprite.player->instances[0].x -= g->p_size_x;
		g->sprite.fo->instances[0].x -= g->p_size_x;
		g->count++;
	}
	if (key.key == MLX_KEY_D && key.action == 1 && check_rigth(g) == 1 && g->success == 0)
	{
		g->sprite.player->instances[0].x += g->p_size_x;
		g->sprite.fo->instances[0].x += g->p_size_x;
		g->count++;
